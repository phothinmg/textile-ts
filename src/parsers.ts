	...
	do {
		...
		if ((m = regexp.reClassid.exec(remaining))) {
			...
			if (
				...
				if (bits?.[0]) {
					const cs = bits[0].trimStart().trimEnd().split(" ");
					if (cs?.length) {
						const css: string[] = [];
						for (let c of cs) {
							if (c.trim().startsWith("*")) {
								c = `language-${c.slice(1)}`;
							}
							css.push(c);
						}
						o.class = css.join(" ");
...
export function parseInline(src: string, options?: Options) {
	...
	do {
		...
		if (
			...
			if (m[4]) {
				// +cite causes image to be warped with a link (or link_ref)?
...
export function parseList(src: string, options?: _Options) {
	...
	while ((m = regexp.reItem.exec(src))) {
		...
		let newLi: string[] | null = null;
		let lst: (string | object | string | string[])[] | null = null;
		let par;
		let pba;
		let r;
		...
		if (itemIndex) {
			...
			// falsy prevents this from firing until it is set again
			itemIndex = 0;
		}
		if (listAttr) {
...
/**
 * Replace any href in links with the corresponding values from the dict.
 * @param ml a JsonML tree
 * @param dict a dictionary of href to replace with
